...
namespace XFS4IoTFramework.GermanSpecific
{
    public partial class SecureMsgReceiveHandler
    {
        private async Task<CommandResult<SecureMsgReceiveCompletion.PayloadData>> HandleSecureMsgReceive(ISecureMsgReceiveEvents events, SecureMsgReceiveCommand secureMsgReceive, CancellationToken cancel)
        {
            ...
            Logger.Log(Constants.DeviceClass, "GermanSpecificDev.SecureMsgReceive()");
            var result = await Device.SecureMsgReceive(
                new(
                ...
                cancel);
            Logger.Log(Constants.DeviceClass, $"GermanSpecificDev.SecureMsgReceive() -> {result.CompletionCode}");
