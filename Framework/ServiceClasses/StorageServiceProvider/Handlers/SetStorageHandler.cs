...
namespace XFS4IoTFramework.Storage
{
    public partial class SetStorageHandler
    {
        private async Task<CommandResult<SetStorageCompletion.PayloadData>> HandleSetStorage(ISetStorageEvents events, SetStorageCommand setStorage, CancellationToken cancel)
        {
            ...
            {
                Dictionary<string, SetCardUnitStorage> cardStorageToSet = [];
                ...
                foreach (var storage in setStorage.Payload.Storage)
                {
                    ...
                    cardStorageToSet.Add(
                        storage.Key, 
                        new SetCardUnitStorage(
                            storage.Value.Card.Configuration is null ? null : new SetCardConfiguration(storage.Value.Card.Configuration.Threshold,
            ...
            if (Storage.StorageType.HasFlag(StorageTypeEnum.Cash))
            {
                ...
                foreach (var storage in setStorage.Payload.Storage)
                {
                    ...
                    if (storage.Value.Cash.Configuration is not null)
                    {
                        ...
                        newConfig = new SetCashConfiguration(
                            types,
                            items,
                    ...
                    if (storage.Value.Cash.Status?.Initial is not null)
                    {
                        ...
                        if (storage.Value.Cash.Status.Initial?.ExtendedProperties?.Count > 0)
                        {
                            ...
                            foreach (var item in storage.Value.Cash.Status.Initial.ExtendedProperties)
                            {
                                ...
                                itemCounts.Add(
                                    item.Key, 
                                    new CashItemCountClass(
                                        item.Value.Fit is null ? 0 : (int)item.Value.Fit,
                        ...
                        newInitialCounts = new StorageCashCountClass(
                            storage.Value.Cash.Status.Initial.Unrecognized is null ? 0 : (int)storage.Value.Cash.Status.Initial.Unrecognized,
                            itemCounts);
                    }
                ...
                Dictionary<string, CashUnitCountClass> countDelta = [];
                foreach (var storageToUpdate in cashStorageToSet)
                {
                    countDelta.Add(storageToUpdate.Key, new(null, null, 0));
                }
                await Storage.UpdateCashAccounting(countDelta);
            ...
            if (Storage.StorageType.HasFlag(StorageTypeEnum.Check))
            {
                ...
                foreach (var storageToUpdate in checkStorageToSet)
                {
                    countDelta.Add(storageToUpdate.Key, new());
                }
            ...
            {
                Dictionary<string, SetPrinterUnitStorage> printerStorageToSet = [];

                foreach (var storage in setStorage.Payload.Storage)
                {
                    if (!Storage.PrinterUnits.ContainsKey(storage.Key))
                    {
                        return new(
                            MessageHeader.CompletionCodeEnum.InvalidData,
                            $"Unrecognised storage ID for printer unit specified. {storage.Key}");
                    }

                    if (storage.Value.Printer is null ||
                        storage.Value.Printer.Status is null ||
                        storage.Value.Printer.Status.Initial is null)
                    {
                        // No configuration or count specified and just ingore this unit
                        continue;
                    }
                    ...
                    printerStorageToSet.Add(
                        storage.Key, 
                        new SetPrinterUnitStorage(InitialCount: storage.Value.Printer.Status.Initial));
                }

                if (printerStorageToSet.Count == 0)
                {
                    return new(
                        MessageHeader.CompletionCodeEnum.InvalidData,
                        $"No card configuration is set to all card units.");
                ...
                Logger.Log(Constants.DeviceClass, "StorageDev.SetPrinterStorageAsync()");

                var result = await Device.SetPrinterStorageAsync(new(printerStorageToSet), cancel);

                Logger.Log(Constants.DeviceClass, $"StorageDev.SetPrinterStorageAsync() -> {result.CompletionCode}, {result.ErrorCode}");

                if (result.CompletionCode == MessageHeader.CompletionCodeEnum.Success)
                {
                    foreach (var storageToUpdate in printerStorageToSet)
                    {
                        Storage.CardUnits.ContainsKey(storageToUpdate.Key).IsTrue($"Unexpected storage ID for card unit found. {storageToUpdate.Key}");

                        if (storageToUpdate.Value.InitialCount is not null)
                        {
                            Storage.PrinterUnits[storageToUpdate.Key].Unit.Status.InitialCount = (int)storageToUpdate.Value.InitialCount;
                            Storage.PrinterUnits[storageToUpdate.Key].Unit.Status.InCount = 0;
                        }
                    }
                }
                ...
                foreach (var storageToUpdate in printerStorageToSet)
                {
                    await Storage.UpdatePrinterStorageCount(storageToUpdate.Key, 0);
                }
