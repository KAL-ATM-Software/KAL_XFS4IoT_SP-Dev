...
namespace XFS4IoTFramework.GermanSpecific
{
    public partial class HSMInitHandler
    {
        private async Task<CommandResult<HSMInitCompletion.PayloadData>> HandleHSMInit(IHSMInitEvents events, HSMInitCommand hSMInit, CancellationToken cancel)
        {
            ...
            Logger.Log(Constants.DeviceClass, "GermanSpecificDev.HSMInit()");
            var result = await Device.HSMInit(
                new(
                ...
                cancel);
            Logger.Log(Constants.DeviceClass, $"GermanSpecificDev.HSMInit() -> {result.CompletionCode}");
