...
namespace XFS4IoTFramework.CardReader
{
    ...
    public partial class RetainCardHandler : ICommandHandler
    {
        ...
        public async Task Handle(IConnection Connection, object command, CancellationToken cancel)
        {
            ...
            retainCardCmd.Header.RequestId.HasValue.IsTrue();

            IRetainCardEvents events = new RetainCardEvents(Connection, retainCardCmd.Header.RequestId.Value);

            var result = await HandleRetainCard(events, retainCardCmd, cancel);
            await Connection.SendMessageAsync(new RetainCardCompletion(retainCardCmd.Header.RequestId.Value, result));
        }

        public async Task HandleError(IConnection connection, object command, Exception commandException)
        {
            var retainCardcommand = command.IsA<RetainCardCommand>();
            retainCardcommand.Header.RequestId.HasValue.IsTrue();
            ...
            RetainCardCompletion.PayloadData.CompletionCodeEnum errorCode = commandException switch
            {
                ...
                TaskCanceledException or OperationCanceledException => RetainCardCompletion.PayloadData.CompletionCodeEnum.Canceled,
                _ => RetainCardCompletion.PayloadData.CompletionCodeEnum.InternalError
            };

            var response = new RetainCardCompletion(retainCardcommand.Header.RequestId.Value, new RetainCardCompletion.PayloadData(errorCode, commandException.Message));
