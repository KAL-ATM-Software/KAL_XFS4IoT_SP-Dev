...
namespace XFS4IoTFramework.CashAcceptor
{
    ...
    {
        private Task<CommandResult<GetDeviceLockStatusCompletion.PayloadData>> HandleGetDeviceLockStatus(IGetDeviceLockStatusEvents events, GetDeviceLockStatusCommand getDeviceLockStatus, CancellationToken cancel)
        {
            if (CashAcceptor.DeviceLockStatus is null)
            {
                return Task.FromResult(new CommandResult<GetDeviceLockStatusCompletion.PayloadData>(
                    MessageHeader.CompletionCodeEnum.UnsupportedCommand,
                    $"The device doesn't support lock status."));
            }

            return Task.FromResult(
                new CommandResult<GetDeviceLockStatusCompletion.PayloadData>(
                    new(
