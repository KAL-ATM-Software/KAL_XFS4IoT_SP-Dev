...
namespace XFS4IoT.Printer.Completions
{
    ...
    public sealed class PrintFormCompletion : Completion<PrintFormCompletion.PayloadData>
    {
        ...
        public sealed class PayloadData : MessagePayload
        {
            ...
            {
                ErrorDescription.IsNotNullOrWhitespace($"Null or an empty value for {nameof(ErrorDescription)} in received {nameof(PrintFormCompletion.PayloadData)}");
            ...
            ////  The specified form definition cannot be found.**flushFail**
            ////  The device was not able to flush data.**mediaOverflow**
            ////  The form overflowed the media.**fieldSpecFailure**
            ////  The syntax of the *fields* member is invalid.**fieldError**
            ...
            ////  The device was not able to turn the inserted media.**shutterFail**
            ////  Open or close of the shutter failed due to manipulation or hardware error.**mediaJammed**
            ////  The media is jammed; operator intervention is required.**charSetData**
            ////  Character set(s) supported by Service Provider is inconsistent with use of *fields*.**paperJammed**
            ////  The paper is jammed.**paperOut**
            ////  The paper supply is empty.**inkOut**
            ////  No stamping possible, stamping ink supply empty.**tonerOut**
            ////  Toner or ink supply is empty or printing contrast with ribbon is not sufficient.**sequenceInvalid**
            ////  Programming error. Invalid command sequence (e.g. *mediaControl* = park and park  position is busy).**sourceInvalid**
            ////  The selected paper source is not supported by the hardware.**mediaRetained**
            ////  Media has been retracted in attempts to eject it. The device is clear and can be used.**blackMark**
            ////  Black mark detection has failed, nothing has been printed.**mediaSize**
            ...
            ////  Presented media was automatically retracted before all wads could be presented and before the  command could complete successfully.**msfError**
            ////  An error occurred while writing the magnetic stripe data.**noMSF**
